  ...
  return (
    <>
      <Image
        ...
        alt="Illustrative view of Karakoram mountains and a river valley inspired by Hunza"
        fill
        priority
      ...
      />
      <div
        className="absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-[#0b2c45]/55 to-transparent"
        aria-hidden="true"
      />
...
export function FeaturedDestinationsSkeleton() {
  return (
    <div
      role="status"
      aria-label="Loading featured destinations"
      className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4"
    >
      {Array.from({ length: 4 }, (_, index) => (
        <Skeleton
          key={index}
          className="aspect-[4/5] rounded-2xl sm:aspect-[4/4.5] lg:aspect-[3/4]"
        />
      ))}
    </div>
  );
...
export async function FeaturedDestinations({ toursPromise }: Props) {
  ...
  if (!featured.length)
    return <p className="body-copy">Destinations are taking a short pause.</p>;
      ...
      {featured.map((tour) => (
        <StaggerItem key={tour.id}>
          ...
          >
            ...
            />
            <span
              className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-[#092b41]/85 via-[#092b41]/25 to-transparent"
              aria-hidden="true"
            />
            <span className="absolute bottom-5 left-5 right-5 flex items-end justify-between gap-2 text-white">
              <span>
...
export async function BeyondImage({ toursPromise }: Props) {
  ...
  return cover && tour ? (
    ...
    <Image
      src="/images/swat.webp"
      alt="Illustrative green valley and river landscape inspired by Swat"
      fill
      sizes="(max-width: 1024px) 100vw, 55vw"
      className="object-cover"
    />
